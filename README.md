...
- void TorqueControl(float torque, float reductionratio): 设置指定电机的扭矩控制。
- void RPMControl(float rpm, float reductionratio): 设置指定电机的转速控制。
- float GetRPM(): 获取电机转速 (RPM)。
- float GetOmega(): 获取电机角速度 (rad/s)。
- float GetCurrent(): 获取电机扭矩电流 (安培)。
- float GetTemp(): 获取电机温度 (摄氏度)。
- float GetAngle(): 获取电机转子绝对角度 (弧度)。
- bool Update(): 更新电机状态。
- void Offline(): 将电机反馈数据清零，用于电机离线状态。
